...
lIn = NNLayer(3,activation=NNLayer.A_Tanh, layerType=NNLayer.T_Input)
...
lH1 = NNLayer(3,activation=NNLayer.A_Tanh, layerType=NNLayer.T_Hidden)
lH2 = NNLayer(2,activation=NNLayer.A_Tanh, layerType=NNLayer.T_Hidden)
lOut = NNLayer(1,activation=NNLayer.A_Tanh, layerType=NNLayer.T_Output)
...
def tanhDer(Y,T):
    YSqrd = np.square(Y)
    return np.divide(
        np.multiply(T, (1 - YSqrd)) - np.multiply(Y, (1 - YSqrd)),
        np.multiply(Y, (1 - Y)))